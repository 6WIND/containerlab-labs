...
# Install Containerlab * TO BE REMOVED WHEN 0.68 is released *
sudo bash -c "$(curl -sL https://get.containerlab.dev)"
#
sudo apt-get install -y git golang-go build-essential
git clone https://github.com/srl-labs/containerlab.git
sed -i -e 's/go 1.23/go 1.23.8/g' containerlab/go.mod
sudo make -C containerlab build
sudo cp containerlab/bin/containerlab /usr/bin/
